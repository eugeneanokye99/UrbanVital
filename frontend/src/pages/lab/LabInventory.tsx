...
export default function LabInventory() {
  // 1. Get global search from Layout
  const { globalSearch, setGlobalSearch } = useOutletContext<{ 
      globalSearch: string; 
      setGlobalSearch: (s: string) => void 
  }>();

  // 2. Local search state (synced with global)
  ...
  const handleDeductStock = (e: React.FormEvent) => {
    e.preventDefault();
    ...
    setInventory(prev => prev.map(item => {
        if (item.id === selectedItem.id) {
            const newQty = item.quantity - usageAmount;
            let newStatus = "Good";
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            <h3 className="text-2xl font-bold text-gray-800">{inventory.filter(i => i.status === "Low Stock").length}</h3>
                    ...
                    <tr key={item.id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button 
                            ...
                            className="bg-orange-50 text-orange-600 border border-orange-200 hover:bg-orange-100 px-4 py-2 rounded-lg text-xs font-bold flex items-center gap-2 ml-auto transition-transform active:scale-95"
                            title="Log Usage"
