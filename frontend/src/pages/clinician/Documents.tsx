...
export default function ClinicianDocuments() {
  ...
  const [isFullscreen, setIsFullscreen] = useState(false); // New State for Mobile Zoom
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className={`xl:col-span-2 order-1 xl:order-2 ${isFullscreen ? 'fixed inset-0 z-50 bg-gray-100 p-4 overflow-y-auto' : 'relative'}`}>
          ...
          <div className={`bg-gray-200/50 p-4 sm:p-6 rounded-3xl border border-gray-200 flex flex-col items-center justify-start min-h-[500px] relative transition-all ${isFullscreen ? 'h-full' : ''}`}>
            ...
            {/* Document Container */}
            <div className={`w-full max-w-[210mm] bg-white shadow-lg transition-transform origin-top ${isFullscreen ? 'scale-100' : 'scale-[0.55] sm:scale-[0.75] md:scale-[0.85] lg:scale-100'}`}>
              <UrbanVitalDocument 
                title={docMeta.title}
