...
const MOCK_RETURNS = [
  { id: "RET-001", date: "2024-01-26", drug: "Paracetamol 500mg", qty: 20, type: "Damaged", reason: "Crushed box on delivery", status: "Approved", batch: "BTC-102" },
  { id: "RET-002", date: "2024-01-25", drug: "Amoxicillin Syrup", qty: 1, type: "Customer Return", reason: "Wrong prescription bought", status: "Pending", batch: "AMC-99" },
  { id: "RET-003", date: "2024-01-24", drug: "Vitamin C Tabs", qty: 50, type: "Expired", reason: "Past expiry date", status: "Disposed", batch: "VIT-22" },
];
  ...
  const handleLogReturn = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      const newReturn = {
        ...
        drug: String(formData.get("drugName")),
        qty: Number(formData.get("qty")),
        type: String(formData.get("type")),
        reason: String(formData.get("reason")),
        batch: String(formData.get("batch")),
        status: "Pending" // Default status needing Admin approval usually
      };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
                ...
                <h3 className="text-2xl font-extrabold text-red-600">{returns.filter(r => r.type === 'Damaged' || r.type === 'Expired').length}</h3>
            </div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
            <div>
                ...
                <h3 className="text-2xl font-extrabold text-blue-600">{returns.filter(r => r.type === 'Customer Return').length}</h3>
            </div>
                    ...
                    {filteredReturns.length === 0 ? (
                        ...
                        filteredReturns.map((item) => (
                            <tr key={item.id} className="hover:bg-blue-50/30 transition-colors">
                                <td className="px-6 py-4 font-mono text-[#073159] font-bold text-xs">{item.id}</td>
                                <td className="px-6 py-4 text-gray-600">{item.date}</td>
                                <td className="px-6 py-4">
                                    <p className="font-bold text-gray-800">{item.drug}</p>
                                    <p className="text-xs text-gray-500">Qty: {item.qty} | Batch: {item.batch}</p>
                                </td>
                                <td className="px-6 py-4">
                                    <span className="block font-medium text-gray-700">{item.type}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
                ...
                <form onSubmit={handleLogReturn} className="p-6 space-y-4">
                    ...
                    {/* Drug Search (Simplified as input for now) */}
                    <div>
                        <label className="text-xs font-bold text-gray-500 uppercase mb-1 block">Drug Name *</label>
                        <input name="drugName" className="w-full p-3 border rounded-xl bg-gray-50 outline-none focus:border-[#073159]" placeholder="Search inventory..." required />
                    </div>
