...
import logo from "../../assets/urbanvital-logo.png"; // Ensure you have a logo asset

export default function PrescriptionReview() {
  const navigate = useNavigate();
  const location = useLocation();
  
  // 1. Get the Updater Function from Context
  const { markAsDispensed } = useOutletContext<{ markAsDispensed: (id: string) => void }>();

  // Data passed from Dashboard
  const { prescription } = location.state || {};

  // Mock Items (In real app, fetch from API based on prescription ID)
  const [items] = useState(
    prescription?.items.map((drug: string, index: number) => ({
      id: index,
      name: drug,
      dosage: "1-0-1 x 5 days", // Mock dosage instructions
  ...
  const handleDispense = () => {
    ...
    // 2. Simulate API Call & Update State
    setTimeout(() => {
      // Call the context function to move item to History
      markAsDispensed(prescription.id);
      
      toast.success(`Prescription #${prescription.id} Dispensed!`);
