        ...
        <main className="flex-1 overflow-y-auto p-4 md:p-6 lg:p-8">
          <div className="max-w-7xl mx-auto space-y-6 md:space-y-8">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <h1 className="text-xl md:text-2xl font-bold text-[#073159] flex items-center gap-2">
                  <DollarSign className="w-6 h-6 text-[#073159]" />
                  Financial Overview
                </h1>
                <p className="text-sm text-gray-500 mt-1">Track revenue, expenses, and clinic profitability.</p>
              </div>
              <div className="flex gap-2">
                <select 
                   value={dateRange}
                   onChange={(e) => setDateRange(e.target.value)}
                   className="bg-white border border-gray-200 text-gray-700 text-sm font-medium px-4 py-2 rounded-xl outline-none cursor-pointer"
                >
                   <option>Today</option>
                ...
                <button className="flex items-center gap-2 px-4 py-2 bg-[#073159] text-white rounded-xl hover:bg-[#062a4d] text-sm font-bold shadow-md">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
              ...
              {/* Left Col: Revenue Breakdown (SVG Chart + Bars) */}
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                <h3 className="font-bold text-lg text-gray-800 mb-6">Revenue Trends & Sources</h3>
                
                {/* Simple SVG Line Chart */}
                <div className="h-48 w-full mb-8 overflow-hidden">
                    <svg viewBox="0 0 600 150" className="w-full h-full" preserveAspectRatio="none">
                        <path d="M0 100 Q 150 20 300 80 T 600 40" fill="none" stroke="#073159" strokeWidth="3" />
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 flex flex-col overflow-hidden">
                    ...
                    {transactions.map((tx) => (
                        <div key={tx.id} className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-gray-100">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-bold text-gray-800">{tx.desc}</p>
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50 text-center">
                    <button className="w-full py-2 text-sm font-bold text-[#073159] border border-gray-200 rounded-lg bg-white hover:bg-gray-100">
                        Add Expense
                    </button>
                </div>
              </div>
